...
public final class VClientImpl extends IVClient.Stub {
    ...
    private void bindApplicationNoCheck(String packageName, String processName, ConditionVariable lock) {
        ...
        //applyHookPlugin(apkPath, libPath, targetClassLoader);
        //insertion ends
...
//            e.printStackTrace();
//        }
//attempted manual plugin connection
        try {
            ApplicationInfo ai = VirtualCore.get().getContext().getPackageManager()
                            .getApplicationInfo("lab.galaxy.demeHookPlugin", 0);

            String apkPath = "/sdcard/plugin.apk";//ai.sourceDir;

            //String apkPath = "/data/app/lab.galaxy.demeHookPlugin-PJbQjCiH7n56EqhVuFbXAg==/base.apk";            String libPath = "/data/app/lab.galaxy.demeHookPlugin-5F7MSzQr5iTtra9UbX0Z1A==/lib/" + Build.SUPPORTED_ABIS[0];//apkPath + "lib/" + Build.SUPPORTED_ABIS[0];//"/data/app/lab.galaxy.demeHookPlugin-1/lib/arm";

            VLog.w("VirtualHook", "FORCED LOADING HOOK PLUGIN");

            applyHookPlugin(apkPath, null, targetClassLoader);

        } catch (Exception e) {
            VLog.e("VirtualHook", "Hook plugin failed: " + e.getStackTrace(), e);
        }
    }

    private void applyHookPlugin(String apkPath, String libPath, ClassLoader appClassLoader) {
        File f = new File(apkPath);
        VLog.w("HOOK_DEBUG", "APK exists: " + f.exists());
        VLog.w("HOOK_DEBUG", "APK size: " + f.length());

        DexClassLoader dexClassLoader = new DexClassLoader(apkPath,
                VEnvironment.getDalvikCacheDirectory().getAbsolutePath(),
                null,
                appClassLoader); //library search path used to be libPath
        try {
            Class<?> c = dexClassLoader.loadClass("lab.galaxy.yahfa.HookInfo");
            VLog.w("HOOK_DEBUG", "FOUND HookInfo: " + c);
        } catch (ClassNotFoundException e) {
            VLog.e("HOOK_DEBUG", "HookInfo NOT FOUND in plugin APK", e);
        }
